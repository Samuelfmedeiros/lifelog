...
<Terminal
  lines={[
  { cmd: 'python3 ~/.hermes/scripts/security-agent.py', output: '(stdout vazio) — nada novo, silêncio é saúde' },
  { cmd: 'cat ~/.hermes/security-agent-state.json', output: '{ "seen_titles": ["...", "..."] } — o que já foi reportado' },
  { cmd: 'tail -20 ~/.hermes/logs/security-agent.log', output: '[ISO] Security Agent — cruzando dados... (SH: ok, BH: ok)' },
  ]}
/>
