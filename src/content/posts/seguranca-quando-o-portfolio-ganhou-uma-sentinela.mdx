...
Cada endpoint ganhou um limitador de requisições em memória: uma janela de um minuto, com um teto de cinco requisições por IP. Depois disso, responde 429 e manda embora. É o suficiente pra um humano baixar o currículo ou mandar uma mensagem — e o suficiente pra travar um bot.

```ts
const rateLimit = new Map<string, { count: number; resetAt: number }>();
const RATE_LIMIT_WINDOW = 60_000; // 1 minuto
const RATE_LIMIT_MAX = 5; // máx por janela
...
Detalhe que evita vazamento de memória: um `setInterval` de limpeza roda a cada cinco minutos e remove entradas expiradas. Um Map que nunca limpa é um leak disfarçado de proteção.

O IP vem de uma cadeia de headers de proxy (`x-forwarded-for`, `x-real-ip`, e o específico da plataforma de deploy), com fallback pra `unknown`. Não é à prova de spoofing sofisticado, mas é o suficiente contra o atacante que temos de verdade: o bot preguiçoso.
...
- **Em produção, semanalmente:** um scanner roda contra a URL pública e verifica, um por um: headers de segurança (HSTS, CSP, proteção de conteúdo, frame, referrer), presença do security.txt, tentativa de acessar arquivos que não deveriam existir (variáveis de ambiente, config do git, config de deploy), o rate limit dos dois endpoints (manda seis requisições e espera a sexta voltar 429), tentativa de path traversal e a validade do certificado TLS.
