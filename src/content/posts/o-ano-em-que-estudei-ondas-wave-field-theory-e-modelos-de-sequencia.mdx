...
O paradoxo: a seção mais "parada" do blog era a que gerava o projeto mais ambicioso. Não porque eu planejava — mas porque **estudo não avisa quando vira código**. Você lê um paper, faz anotação no vault, testa um notebook, e seis meses depois aquilo vira o kernel de travessia que roda em microssegundos por token.
...
| Mamba SSM (estudo + implementação) | Modelo 1.4B, inferência em µs/tok | GPU pipeline funcional |
