...
O mesmo bug aparecia em **4 campos** (chave Pix, Merchant Name, Merchant City, e o CRC que usava `slice(0, -4)` em vez de concatenar o CRC no final). O commit `<commit>` corrigiu todos.
...
git show <commit>  # fix: focus trap roubando foco ao digitar
...
git show <commit>  # navigationRef pausa observer durante smooth-scroll
git show <commit>  # handleNavClick seta activeSection no click
```

### +6 problemas no mesmo dia

O commit `refactor: auditoria profunda — 10 correções de qualidade` (`<commit>`) consolidou:
...
A mensagem do commit `<commit>` é longa de propósito — cada correção tem o _o quê_ e o _por quê_. Debugging futuro agradece.
...
<Terminal
  lines={[
  { cmd: 'git show <commit> --stat', output: 'Pix BR Code fix — 2 arquivos, 26 inserções' },
  { cmd: 'git log --oneline <commit>..HEAD | grep fix | wc -l', output: 'Total de fixes pós-auditoria' },
