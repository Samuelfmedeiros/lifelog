...
hide_dotdirs = ["<protected-dir>"]  # the agent workdir becomes invisible in the sandbox
deny_paths = ["secrets/", "*.key", ".gnupg/"]
```

The `hide_dotdirs` was the fine discovery: hiding the agent's entire working directory inside the sandbox, so neither the agent nor a prompt injection can even *see* it exists — what doesn't show up in `ls` can't leak through exfiltration.
...
| Security | agent without sandbox | ai-jail (mask + isolation) + Sandbox 51/51 |
...
3. **Agent security is a layer, not a feature** — sandbox, secret masking and directory isolation protect against the worst case (prompt injection), not against the user.
...
{ cmd: 'grep -c hide_dotdirs ~/.ai-jail', output: 'present — agent dir invisible in the sandbox' },
  ]}
/>
