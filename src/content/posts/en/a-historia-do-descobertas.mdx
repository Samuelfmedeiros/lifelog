...
The fourth discovery: you can run LLMs locally without depending on the cloud. **Ollama on WSL + Douglas** (the mirror PC) runs `qwen3.5-vision:latest` — I just verified, it's online:
...
With that, vision (VLM), generation and analysis run at home — low latency, zero cost per token, data that never leaves the network. Arachne consumes Douglas's Ollama as a VLM fallback, and TatuEngine runs BitMamba-2 1B on local GPU with 252× speedup.
...
| AI | cloud-only | Local Ollama (Douglas, qwen3.5-vision) |
...
<Terminal
  lines={[
    ...
    { cmd: 'curl -s http://192.168.7.5:11434/api/tags', output: 'qwen3.5-vision:latest — local Ollama on Douglas' },
