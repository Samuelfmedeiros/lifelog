...
<Terminal
  commands={[
    { label: 'rate', cmd: 'curl -s -o /dev/null -w "%{http_code}" -X POST <your-site>/api/<endpoint> -d "{}" -H "Content-Type: application/json"' },
    { label: 'well-known', cmd: 'curl -s <your-site>/.well-known/security.txt' },
