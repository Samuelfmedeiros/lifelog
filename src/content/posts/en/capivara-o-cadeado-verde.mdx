...
<Terminal
  ...
  { cmd: 'git log --oneline -1', output: 'fix(critical): path traversal in SPA — production secret leak' },
  { cmd: 'git log --oneline -1', output: 'fix(sec): defense-in-depth hardening on the _next/static proxy' },
  { cmd: 'git log --oneline -1', output: 'fix(sec): path traversal in Umami proxy — normalize path before' },
