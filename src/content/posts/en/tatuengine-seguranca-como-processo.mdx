...
TatuEngine had already done a lot by now: it learned to **infer** (BitMamba-2 1B on GPU, 252× speedup), learned to **act** (autopoietic agent with 8 subsystems, ToolUse, GoalStack, LTM), and was learning to **learn** (Master-Apprentice loop). Underneath all of that, a **hybrid sandbox** with 51/51 path-validation tests.
...
3. **Sandbox is the frontier** — AI tool execution only through `tatu/sandbox.py`
