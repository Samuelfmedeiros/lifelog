...
LifeLog (the Astro blog) had suffered the **same problem** a few days earlier — the cache got stuck on an old commit for days, until someone noticed. It was fixed on June 22nd with `no-cache` on the entrypoints.
