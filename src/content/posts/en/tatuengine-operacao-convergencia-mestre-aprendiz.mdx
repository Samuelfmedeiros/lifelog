...
This is where the saga almost died. The student **wasn't converging** — the answers didn't follow the format, alignment kept breaking. Two commits fixed it:

```bash
# v0.21.5 (6bc95bb) — Operation Convergence: forced semantic mining
feat(convergence): v0.21.5 — Operação Convergência (mineração semântica forçada)

# v0.22.0 (c06eac1) — +505 lines in one commit: the teacher system message
# was breaking the student's alignment
feat(convergence): v0.22.0 — Força de Convergência (teacher system message fix)
```

The villain was the **teacher's system message**: the instruction the teacher received contaminated the distillation. Fixing the professor's prompt (+505 lines of fix) unlocked convergence.
...
- **Trained teacher LoRA** — Qwen2.5-3B-Instruct, R=16, alpha=32, targets q/k/v/o (`tatu_lora_train.py`)
- **Working distillation** — 381 examples generated with the `[THOUGHT]/[ANSWER]` protocol (`tatu_distill.py`)
- **3 consolidated datasets** — `tatu_train_data` (349) + `student_train` (23) + `distill_clean_train` (9)
- **Differentiable PyTorch student** — `bitmamba_pt.py`, 1B with forward + backward validated (RTX 3060, CUDA 13.0), gradients flowing
...
| Convergence fix | teacher system message (+505 lines, v0.22.0) |
...
<Terminal
  lines={[
    { cmd: 'git show c06eac1 --stat', output: 'v0.22.0 — +505 lines: teacher system message fix' },
    { cmd: 'python3 tatu_distill.py', output: '381 [THOUGHT]/[ANSWER] examples distilled' },
    { cmd: 'python3 bitmamba_pt.py', output: 'Student 1B differentiable — forward + backward OK (RTX 3060)' },
    { cmd: 'python3 tatu_sft_train.py', output: 'Student SFT — 3 epochs, 381 examples (next milestone)' },
  ]}
/>
