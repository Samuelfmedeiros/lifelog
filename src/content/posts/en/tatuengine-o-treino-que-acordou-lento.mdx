...
```text
restore scheduler state from the checkpoint
for each parameter group:
    group_base_lr = learning_rate_declared_in_this_session   # the session owns it, not the checkpoint
confirm the re-imposed value in the log
if restoring fails, use a fresh scheduler
```
...
<Terminal
  title="fix-summary"
  commands={[
    { cmd: 'git log --oneline -1', output: 'fix: re-impose scheduler LR after ckpt load (old ckpt carried LR ~0)' },
    { cmd: 'tail -2 telemetry.log | grep lr', output: 'before the fix: tiny LR inherited from the old checkpoint' },
    { cmd: 'tail -2 telemetry.log | grep lr  # after the fix', output: 'after: LR = 1.00e-04, exactly what the CLI asked for' },
  ]}
/>
