...
project: seguranca
tags: [seguranca, ai-jail, watchdog, hardening, agents, security]
...
The fine detail: `hide_dotdirs` — hiding the agent's **entire** directory inside the sandbox. Neither the agent nor a prompt injection can list what doesn't exist for them.
...
1. **Agent security is a layer, not a feature** — sandbox, mask and directory isolation protect
...
<Terminal
  lines={[
  { cmd: 'cat ~/.<protected-dir>/secrets.env', output: 'permission denied — mask applied in the sandbox' },
