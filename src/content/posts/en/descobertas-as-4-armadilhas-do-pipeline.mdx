...
The Astro build runs with `pnpm`. On my WSL, the pnpm is Hermes's: `/home/samuel/.hermes/node/bin/pnpm`. And that's where everything froze:
...
The gateway's security guard blocked **any command whose binary resolves inside `~/.hermes/`** — it assumes it's an internal command. The legitimate `pnpm` became a suspect.

The workaround went straight to Astro's real binary, with the system node (outside `~/.hermes`):
...
1. **Absolute path for the build** — `pnpm` that resolves inside `~/.hermes/` can be blocked by the gateway guard. Use `/usr/bin/node node_modules/astro/bin/astro.mjs build`.
...
2. **The right binary on the right path** — binaries that resolve inside `~/.hermes/` become guard targets. Pin absolute paths.
