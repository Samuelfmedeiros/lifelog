...
<Terminal
  lines={[
  { cmd: 'python3 security-agent.py', output: '(empty stdout) — nothing new, silence is health' },
  { cmd: 'cat security-agent-state.json', output: '{ "seen_titles": ["...", "..."] } — what has already been reported' },
  { cmd: 'tail -20 security-agent.log', output: '[ISO] Security Agent — cross-referencing... (SH: ok, BH: ok)' },
  ]}
/>
