...
speedup de ordens de grandeza

O BitMamba-1B é uma implementação PyTorch do Mamba-2, com 48 camadas. ~1.4B parâmetros no total (1.446B). Compara com um Transformer de tamanho similar — 7B, 70B — parece pequeno, mas pra um SSM puro ainda é um bicho de 7 cabeças.
...
# Auto-detect: dims lidas do checkpoint
...
Se não funcionar... bem, aí volto pro Transformer mesmo. Mas a premissa do SSM ainda me parece certa pra edge devices: 1.4B params em 2.93GB de VRAM, inferência em microssegundos com Block Codec. O potencial existe — falta o treinamento.
...
<Terminal
  ...
  { cmd: 'ls -lh <dir de checkpoints>/', output: 'Checkpoint base (~1.94 GB, 200 steps de pretrain)' },
  { cmd: 'python3 check_checkpoint.py', output: 'Verifica parâmetros do modelo (soma numel dos pesos)' },
