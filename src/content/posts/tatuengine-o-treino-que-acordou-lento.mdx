...
```text
restaurar estado do scheduler a partir do checkpoint
para cada grupo de parametros:
    base_lr_do_grupo = lr_declaração_da_sessao   # a sessão manda, não o checkpoint
confirmar no log o valor efetivo reimpreso
se restaurar nao der, usar scheduler fresco
```
...
<Terminal
  ...
  commands={[
    { cmd: 'git log --oneline -1', output: 'fix: reforcar a LR do scheduler apos load de ckpt (ckpt antigo carregava LR ~0)' },
    { cmd: 'tail -2 telemetria.log | grep lr', output: 'antes do fix: LR minúscula herdada do checkpoint antigo' },
    { cmd: 'tail -2 telemetria.log | grep lr  # apos o fix', output: 'depois: LR = 1.00e-04, exatamente a pedida no CLI' },
  ]}
/>
