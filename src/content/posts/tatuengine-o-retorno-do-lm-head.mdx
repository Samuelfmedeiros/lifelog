...
A auditoria gerou um checkpoint novo — o mesmo modelo, mas com a cabeça re-escalada e saudável.

E foi exatamente esse artefato corrigido que o script da Fase 4 adotou como base de carga.

```text
# base do modelo: checkpoint corrigido da auditoria (arquivo interno)
base = checkpoint corrigido da auditoria
```

A Fase 4 — batizada de Ressurreição, autorizada pelo arquiteto em 28/08 — precisa de uma **carga limpa**: um checkpoint de pré-treino sem SFT contaminando a memória. O checkpoint original tinha a cabeça com escala derivada; o novo é a versão corrigida. Sem a auditoria, a Fase 4 teria começado com uma cabeça doente e os primeiros steps de treino estariam brigando contra a escala errada.

A ordem de serviço da Fase 4:

```text
1. Carga limpa: checkpoint de pre-treino, sem SFT
2. AdamW (Fused) + Parameter Groups: learning rate diferenciada por grupo — os parâmetros de estado do SSM num regime, o corpo do modelo em outro
3. Loop Híbrido: fase de pre-treino puro, depois co-training pre-treino + SFT
4. Telemetria: perdas separadas (pre-treino vs SFT) desde o início do co-training
```
...
| Métrica | Valor |
|---------|-------|
| Checkpoint original | ~2 GB, escala derivada na cabeça |
| Checkpoint corrigido | ~2 GB, lm_head re-escalado |
...
| Base da Fase 4 | checkpoint corrigido — carga limpa |
...
<Terminal
  lines={[
  { cmd: "python3 lm_head_fix.py", output: "lm_head: re-escala Xavier OK · perda esperada de cabeça nova · grad OK · checkpoint salvo" },
  { cmd: "ls -lh models/", output: "2.0G checkpoint corrigido — salvo, pronto p/ Fase 4" },
  { cmd: "python fase4.py --dry-run", output: "base: checkpoint corrigido da auditoria" },
  ]}
/>
