...
<Terminal
  lines={[
  { cmd: 'python3 -c "import json; j=json.load(open(\\'~/.hermes/cron/jobs.json\\')); print(len(j.get(\\'jobs\\', j)), \\'total jobs\\')"', output: '139 — depois da limpeza (era 289)' },
  { cmd: 'git log --oneline -1 971cb97', output: 'O commit do fix do Yurumi — 10.7s de consolidação' },
  { cmd: 'python3 -c "import numpy as np,time; v=np.random.rand(2000,768); v=v/np.linalg.norm(v,axis=1,keepdims=True); t=time.time(); s=np.dot(v,v.T); print(round(time.time()-t,2), \'s — matmul 2000x2000\')"', output: '1.8s — contra 30min do sklearn pair-wise' },
  ]}
/>