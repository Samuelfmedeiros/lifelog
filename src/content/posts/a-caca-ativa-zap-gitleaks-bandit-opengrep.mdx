...
A caça ativa virou **processo contínuo**. Criei o `security-watchdog.py` que roda **todo dia às 07:30** e fica **silencioso quando está tudo OK** — só grita no canal Notificações quando encontra problema.
...
# Summary: ✅ All clean  |  ⚠️ Warnings  |  🚨 Critical → alerta Telegram
```
...
| Scan CI bloqueando deploy | ✅ Ativo (falha = não entrega) |
| Watchdog diário | ✅ 07:30, silent unless issues |
...
<Terminal
  ...
  { cmd: 'python3 security-watchdog.py', output: '✅ All clean — monitorando 24/7, alerta só em problema' },
  ]}
/>