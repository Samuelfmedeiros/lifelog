...
project: tatuengine
tags: [tatuengine, update, progress, ssm, gpu]
...
O TatuEngine passou por uma transformação radical nos últimos 30 dias. Do kernel de travessia com speedup de centenas de vezes ao pipeline completo de Teacher-Student, o que começou como "e se RT Cores rodassem redes neurais?" virou um motor de inferência funcional com agente autopoiético, sandbox de segurança, e 128 testes Python passando (sem contar os 40+ testes C++).
...
O marco mais pesado foi o pipeline de Supervised Fine-Tuning. O Teacher LoRA (Qwen2.5-3B-Instruct em GGUF) terminou o treino, e 381 exemplos foram destilados em datasets. O Student (BitMamba-1B PyTorch) tá pronto, diferenciável, forward+backward validados na RTX 3060 — mas ainda não treinado.

O Teacher virou um `.gguf` de 2GB. A fila da SFT full é o próximo passo — o Student ainda não aprendeu nada.

### Sandbox de segurança: níveis de isolamento

O sandbox entrou em produção com **níveis de permissão**: SAFE, PROMPT, RESTRICTED, DENIED. Bloqueia escrita em filesystem, chamadas perigosas, argv malicioso. 51/51 testes só no módulo de sandbox.

Cada tool declara seu nível de sandbox. A execução de ferramentas valida antes de executar — se uma ferramenta SAFE tenta acessar um path proibido, o sandbox barra.

### Depth Pruning: Mamba2 48 → 32 layers

O pruning foi um experimento interessante: pegar o Mamba2-1.3B original (48 layers) e remover cirurgicamente as 16 últimas camadas de abstração. O rationale? Nem toda profundidade traduz em qualidade pra tasks específicas — e 32 layers ocupam 33% menos VRAM.
...
**4. Softplus com float32 estoura perto do limite do tipo.** `exp()` vira `inf`, o backward espalha NaN. Um patch de 1 linha com a formulação numericamente estável resolveu.
