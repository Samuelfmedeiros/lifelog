...
O LifeLog (blog Astro) tinha sofrido do **mesmo problema** uns dias antes — o cache ficou preso num commit antigo por dias, até alguém notar. Foi corrigido em 22 de junho com cache-control `no-cache` nos entrypoints.
...
<Terminal
  commands={[
  { cmd: 'cd ~/projetos/portifolio && git log --oneline -5', output: 'fix(cache): no-cache no HTML + meta tags + guard bfcache\nfix(deploy): remove node-linker=hoisted\nfix(deploy): packageManager pnpm@10.34.5\nchore: issue/PR templates + contributing guide\ndocs: add MIT license' },
