...
project: portfolio
tags: [portfolio, view-transitions, react, flushsync, animation, theme, accessibility]
...
Foi um commit de 14 linhas que resolveu um problema que existia ha meses. A linha chave:
...
A restauracao da View Transition circular veio acompanhada de um mutirao de qualidade no mesmo dia:
  ...
  commands={[
    {
      ...
      output: 'COMMIT_REF fix(theme): flushSync no startViewTransition + origem do circulo no clique\nCOMMIT_REF feat: restaura View Transition circular no toggle de tema + botoes icon-only\n1192e65 fix: hydration mismatch #418 no carregamento (MissionClock + Footer)'
    },
    {
