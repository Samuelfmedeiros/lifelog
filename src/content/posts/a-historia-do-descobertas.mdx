...
hide_dotdirs = ["<diretorio-protegido>"]  # o dir do agente fica invisivel no sandbox
deny_paths = ["secrets/", "*.key", ".gnupg/"]
```

O `hide_dotdirs` foi a descoberta fina: esconder o diretório de trabalho do agente inteiro dentro do sandbox, pra nem o agente nem um prompt injection conseguirem nem *ver* que ele existe — o que não aparece no `ls` não vaza por exfiltração.
...
| Segurança | agente sem sandbox | ai-jail (mask + isolation) + Sandbox 51/51 |
...
3. **Segurança de agente é camada, não feature** — sandbox, mask de secrets e isolamento de diretório protegem contra o pior caso (prompt injeção), não contra o usuário.
...
{ cmd: 'grep -c hide_dotdirs ~/.ai-jail', output: 'presente — dir do agente invisivel no sandbox' },
  ]}
/>
