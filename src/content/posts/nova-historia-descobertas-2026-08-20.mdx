...
project: descobertas
tags: [descobertas, lifelog, navbar, css, refactoring]
...
O LifeLog sempre teve a navbar começando pelo logo `[LifeLog · Samuel]`. Foi o primeiro a ser removido. O commit resumia a intenção:
...
O quarto e último commit foi o que finalmente resolveu a estrutura do selecionador de paleta:
...
<Terminal commands={[
  ...
  { cmd: 'git show COMMIT_REF --stat   # remove rail-lang e logo' },
  { cmd: 'git show COMMIT_REF --stat   # polish: hover, focus-visible, transitions' },
  { cmd: 'git show COMMIT_REF --stat   # PalettePicker e tema global' }
]}/>