...
O detalhe fino: `hide_dotdirs` — esconder o diretório **inteiro** do agente dentro do sandbox. Nem o agente nem um prompt injection conseguem listar o que não existe pra eles.
...
1. **Segurança de agente é camada, não feature** — sandbox, mask e isolamento de diretório protegem
...
<Terminal
  lines={[
  { cmd: 'cat ~/.<dir-protegido>/secrets.env', output: 'permission denied — mask aplicado no sandbox' },
