...
<Terminal
  lines={[
    { cmd: 'cd ~/projetos/TatuEngine && python3 tatu/scripts/tatu_sft_train.py --help', output: 'Pipeline SFT completo — argumentos e defaults' },
    { cmd: 'nvidia-smi --query-gpu=memory.total,memory.used --format=csv', output: 'Verifica VRAM disponível (alvo: 12 GB RTX 3060)' },
    { cmd: 'python3 -c "from tatu.models.bitmamba import BitMamba; m=BitMamba(); print(f\'{sum(p.numel() for p in m.parameters())/1e9:.2f}B params\')"', output: 'Confirma arquitetura do modelo (1.446B params)' },
    { cmd: 'ls -lh data/train/student_train.jsonl', output: 'Dataset SFT com 381 exemplos destilados do Teacher LoRA' },
    { cmd: 'grep -rn "grad_clamp\\|state_lr_scale\\|Adafactor" tatu/scripts/tatu_sft_train.py', output: 'Verifica se os 4 fixes estão no código' },
  ]}
/>
