---
title: "O Capivara que não cai — espelho na máquina de backup e o failover que funcionou sozinho"
description: "O hub pessoal devolvia 502 intermitente no tunnel. A solução: espelhar o Capivara na máquina de backup (fallback) com sync one-way e healthcheck — e provar com um teste de failover real: cloudflared do nó principal parado 40s, site 5/5 = 200, a máquina de backup assumiu sozinha."
...
4. **Tunnel do nó de backup** — `capivara.seu.pet → 127.0.0.1:8001` no cloudflared dele
...
6. **Healthcheck** — timer de 5min no nó de backup pra garantir que o serviço tá vivo
...
<Terminal
  lines={[
  { cmd: '~/.config/hermes/scripts/capivara-db-sync-douglas.sh', output: 'snapshot sqlite3 .backup → scp → swap no backup' },
  { cmd: 'curl -s -o /dev/null -w "%{http_code}" https://capivara.seu.pet', output: '200 — backup assumindo durante o failover' },
  { cmd: 'git log --oneline -3', output: '9477700 docs+req: espelho backup do Capivara (2026-08-03)' },
  { cmd: 'systemctl --user restart capivara-backend.service', output: 'restart do backend no principal' },
  ]}
/>
