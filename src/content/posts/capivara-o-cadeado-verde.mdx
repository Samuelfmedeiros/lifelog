...
<Terminal
  ...
  { cmd: 'git log --oneline -1', output: 'fix(critical): path traversal no SPA — LEAK de segredos em producao' },
  { cmd: 'git log --oneline -1', output: 'fix(sec): hardening defense-in-depth no proxy _next/static' },
  { cmd: 'git log --oneline -1', output: 'fix(sec): path traversal no proxy Umami — normalizar path antes' },
